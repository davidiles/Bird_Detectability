...
for (i in 1:nrow(result_df)){
  ...
  ylim <- range(result_df[,c("phi_est","phi")],na.rm = TRUE)
  
  result_summary <- result_df %>%
    group_by(time_bins) %>%
    summarize(phi_est = mean(phi_est))
  if ((sim_rep %% 10) == 0) {
  result_plot <- ggplot(data = result_df)+
    geom_jitter(aes(x = time_bins, y = phi_est), height = 0, width = 0.05, col = "gray70")+
    geom_abline(slope=1,intercept=0)+
    geom_point(data = result_summary, aes(x = time_bins, y = phi_est), size = 5, col = "dodgerblue")+
