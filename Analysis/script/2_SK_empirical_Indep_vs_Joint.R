...
unique(species_to_include$Species)
...
# Date/time of each survey
dat$DateTime <- mdy(dat$SurveyDate) + hours(hour(parse_date_time(dat$Time, '%I:%M:%S %p')))
tz(dat$DateTime) <- "Canada/Central"
dat$Hour <- hour(dat$DateTime)
dat$Day <- yday(dat$DateTime)
