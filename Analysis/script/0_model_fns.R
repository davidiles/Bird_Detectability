  ...
  nll.fun <- function(params) {
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * (1-exp(-phi*tmax*exp(-(dmax/tau)^2))),
                               list(phi = phi_k, 
                                    tau = tau_k,
                                    tmax = tmax))
        eval(integrand)
...
fit_IJPP_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
  ...
  if (!is.null(X3)){
    X3 <- X3[Ykeep, ]
    pA_params <- colnames(X3)
  } else {
    X3 <- matrix(1,nrow = nsurvey,ncol=1)
    pA_params <- colnames(X3)[1] <- "logit_pA"
  }
  ...
  if (length(pA_inits) != ncol(X3)) pA_inits <- NULL
  if (is.null(pA_inits)) {
    pA_inits <- rep(0, ncol(X3))
    names(pA_inits) <- pA_params
  }
  
  inits <- c(tau_inits,phi_inits,pA_inits)
  
  # Function to calculate multinomial cell probabilities for each point count
  nll.fun <- function(params) {
    
    tau <- poisson("log")$linkinv(drop(X1 %*% params[1:length(tau_params)]))
    phi <- poisson("log")$linkinv(drop(X2 %*% params[(length(tau_params) + 1):(length(tau_params) + length(phi_params))]))
    pA <-   binomial("logit")$linkinv(drop(X3 %*% params[((length(tau_params) + length(phi_params)) + 1):length(params)]))
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * ( pA*exp(-(dmax/tau)^2)*(1-exp(-phi*tmax)) + (1-pA)*(1-exp(-phi*tmax*exp(-(dmax/tau)^2))) ),
                               list(tau = tau_k,
                                    phi = phi_k, 
                                    pA = pA_k,
                                    tmax = tmax))
...
# *************************************************************************************
# Mixture model with 5 parameters
# *************************************************************************************

fit_IJPP5_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                          rarray, # distance intervals for each point count
                          tarray, # time intervals for each point count
...
offsets_IJPP5_fn <- function (Yarray = Yarray,
                              rarray, # distance intervals for each point count
                              tarray, # time intervals for each point count
...
fit_IJmix_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                          rarray, # distance intervals for each point count
                          tarray, # time intervals for each point count
  ...
  if (!is.null(X2)){
    X2 <- X2[Ykeep, ]
    phi_A_params <- colnames(X2)
  } else {
    X2 <- matrix(1,nrow = nsurvey,ncol=1)
    phi_A_params <- colnames(X2)[1] <- "log_phi_A"
  }
  
  if (!is.null(X3)){
    X3 <- X3[Ykeep, ]
    phi_B_params <- colnames(X3)
  } else {
    X3 <- matrix(1,nrow = nsurvey,ncol=1)
    phi_B_params <- colnames(X3)[1] <- "log_phi_B"
  }
  ...
  if (length(phi_A_inits) != ncol(X2)) phi_A_inits <- NULL
  if (is.null(phi_A_inits)) {
    phi_A_inits <- rep(0, ncol(X2))
    names(phi_A_inits) <- phi_params
  }
  
  if (length(phi_B_inits) != ncol(X3)) phi_B_inits <- NULL
  if (is.null(phi_B_inits)) {
    phi_B_inits <- rep(0, ncol(X3))
    names(phi_B_inits) <- phi_params
  }
  ...
  inits <- c(tau_A_inits,phi_A_inits,phi_B_inits,pA_inits)
  
  # Function to calculate multinomial cell probabilities for each point count
  nll.fun <- function(params) {
    
    tau_A <- poisson("log")$linkinv(drop(X1 %*% params[1:length(tau_A_params)]))
    phi_A <- poisson("log")$linkinv(drop(X2 %*% params[(length(tau_A_params) + 1):((length(tau_A_params) + length(phi_A_params)))]))
    phi_B <- poisson("log")$linkinv(drop(X3 %*% params[(length(tau_A_params) + length(phi_A_params) + 1):((length(tau_A_params) + length(phi_A_params) + length(phi_B_params)))]))
    pA <-   binomial("logit")$linkinv(drop(X4 %*% params[(length(tau_A_params) + length(phi_A_params) + length(phi_B_params) + 1):length(params)]))
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * ( pA*exp(-(dmax/tau_A)^2)*(1-exp(-phi_A*tmax)) + (1-pA)*exp(-(dmax/tau_A)^2)*(1-exp(-phi_B*tmax)) ),
                               list(tau_A = tau_A_k,
                                    phi_A = phi_A_k, 
                                    phi_B = phi_B_k, 
                                    pA = pA_k,
...
offsets_IJmix_fn <- function (Yarray = Yarray,
                              rarray, # distance intervals for each point count
                              tarray, # time intervals for each point count
...
fit_PPmix_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                          rarray, # distance intervals for each point count
                          tarray, # time intervals for each point count
  ...
  nll.fun <- function(params) {
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * (   pA*(1-exp(-phi_A*tmax*exp(-(dmax/tau_A)^2))) + (1-pA)*(1-exp(-phi_B*tmax*exp(-(dmax/tau_A)^2))) ),
                               list(tau_A = tau_A_k,
                                    phi_A = phi_A_k, 
                                    phi_B = phi_B_k, 
                                    pA = pA_k,
...
offsets_PPmix_fn <- function (Yarray = Yarray,
                              rarray, # distance intervals for each point count
                              tarray, # time intervals for each point count
