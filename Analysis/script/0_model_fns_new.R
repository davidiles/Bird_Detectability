...
fit_Ind_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
  ...
  if (optimize == TRUE) res <- optim(inits, nll.fun, method = method, hessian = TRUE) else{
    res <- data.frame("convergence"=NA,"par"=NA,"value" = NA)
  }
  
  # Calculate likelihood with initial values (for comparison)
  res_init <- nll.fun(inits) %>% as.numeric()
  
  rval <- list(convergence = res$convergence,
               coefficients = res$par,  
               loglik = -res$value,
               loglik_with_inits = -res_init)
...
offsets_Ind_fn <- function (Yarray = Yarray,
                           rarray, # distance intervals for each point count
                           tarray, # time intervals for each point count
...
fit_Jnt_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                       rarray, # distance intervals for each point count
                       tarray, # time intervals for each point count
...
offsets_Jnt_fn <- function (Yarray = Yarray,
                           rarray, # distance intervals for each point count
                           tarray, # time intervals for each point count
...
# *************************************************************************************
# JOINT Mixture model with 5 parameters
# *************************************************************************************

fit_JntMix5_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                          rarray, # distance intervals for each point count
                          tarray, # time intervals for each point count
  ...
  nll.fun <- function(params) {
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * ( pA*(1-exp(-phi_A*tmax*exp(-(dmax/tau_A)^2))) + (1-pA)*(1-exp(-phi_B*tmax*exp(-(dmax/tau_B)^2))) ),
                               list(tau_A = tau_A_k,
                                    tau_B = tau_B_k,
                                    phi_A = phi_A_k, 
                                    phi_B = phi_B_k, 
...
offsets_JntMix5_fn <- function (Yarray = Yarray,
                              rarray, # distance intervals for each point count
                              tarray, # time intervals for each point count
...
# *************************************************************************************
# INDEP Mixture model with 5 parameters
# *************************************************************************************

fit_IndMix5_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                           rarray, # distance intervals for each point count
                           tarray, # time intervals for each point count
  ...
  nll.fun <- function(params) {
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * ( pA*(exp(-(dmax/tau_A)^2)*(1-exp(-phi_A*tmax))) + (1-pA)*(exp(-(dmax/tau_B)^2)*(1-exp(-phi_B*tmax)))  ),
                               list(tau_A = tau_A_k,
                                    tau_B = tau_B_k,
                                    phi_A = phi_A_k, 
                                    phi_B = phi_B_k, 
...
offsets_IndMix5_fn <- function (Yarray = Yarray,
                               rarray, # distance intervals for each point count
                               tarray, # time intervals for each point count
...
# *************************************************************************************
# INDEP Mixture model with 4 parameters
# *************************************************************************************

fit_IndMix4_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                           rarray, # distance intervals for each point count
                           tarray, # time intervals for each point count
...
offsets_IndMix4_fn <- function (Yarray = Yarray,
                               rarray, # distance intervals for each point count
                               tarray, # time intervals for each point count
...
fit_JntMix4_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
                           rarray, # distance intervals for each point count
                           tarray, # time intervals for each point count
...
offsets_JntMix4_fn <- function (Yarray = Yarray,
                               rarray, # distance intervals for each point count
                               tarray, # time intervals for each point count
