...
result_df <- expand.grid(phi = c(1,2,5),
                         tau = 1,
                         reps = 1:1000,
                         Density_Est = NA)
...
for (i in 1:nrow(result_df)){
  ...
  for (s in 1:n_surveys){
    ...
    # Each bird has a uniform cue interval over time, but different birds have different rates
    intervals_per_bird <- rexp(n,phi)
    
    start_per_bird <- runif(n,-100,0)
    interval_per_bird <- rep(1/phi,n) #rnorm(n,1/phi,1/phi * 0.1)
    for (bird_id in 1:n) cues[bird_id,] <- start_per_bird[bird_id] + cumsum(rep(interval_per_bird[bird_id],ncol(cues))) 
  ...
  plot1 <- ggplot(result_df)+geom_histogram(aes(x = Density_Est)) + theme_bw()+ facet_grid(phi~.)
  print(plot1)
}
