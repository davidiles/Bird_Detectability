...
nll_fn <- function(tau_est, phi_est){
  ...
  nll <- -sum(sapply(k:nsurvey, function(i) {logdmultinom(Y_array[k, ,], Ysum[k], p_array[k,,])}))
...
for (sim_rep in 1:100){
  ...
  Y_array <- array(NA,dim=c(nsurvey,nrint,ntint))
  
  for (k in 1:nsurvey){
    
    N = round(runif(1,2,1000)) # Number of birds to place on landscape (select a high number to provide sufficient sample size)
  ...
  # ******************************************
  Ysum <- apply(Y_array, c(1), sum)
  Ysum
  
  
  # For QPAD estimation, drop sites with total counts of zero
  # (conditional MLE)
