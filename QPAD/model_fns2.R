...
fit_PP_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
  ...
  nll.fun <- function(params) {
    ...
    phi <- poisson("log")$linkinv(drop(X2 %*% params[(length(tau_params)+length(phi_params) + 1):(length(params)-1)]))
...
fit_IJ_fn <- function (Yarray, # Array with dimensions (nsurvey x nrint x ntint)
  ...
  inits <- c(tau_A_inits,tau_B_inits,phi_inits, gamma_inits)
  ...
  nll.fun <- function(params) {
    ...
    phi <- poisson("log")$linkinv(drop(X3 %*% params[(length(tau_A_params)+length(tau_B_params) + 1):(length(params)-1)]))
    gamma <- params[length(params)]
    ...
    for (k in 1:nsurvey){
      ...
      f_d = function(dmax){
        ...
        integrand = substitute(2*pi*dmax * (exp(-(dmax/tau_A)^2)*(1-exp(-phi*tmax^gamma))),
                               list(phi = phi_k, 
                                    tau_A = tau_A_k,
                                    tau_B = tau_B_k,
                                    gamma = gamma,
