...
#
# This script uses the full matrix of distance and time bins to fit models
#  - a second script will use only the marginal counts
# ********************************************************
...
sink("analysis_distance_time_matrix.jags")
cat("
    model {
   ...
   # Difference within each distance bin to calculate cell probabilities
    ...
    # Place counts in vectorized bins for use with dmulti()
...
for (sim_rep in 1:25){
  ...
  out <- jags(data = jags_data,
              model.file =  "analysis_distance_time_matrix.jags",
  ...
  tau_plot <- ggplot(data = results_df, aes(x = sim_rep, y = tau_est_q50, ymin = tau_est_q025, ymax = tau_est_q975))+
    geom_hline(data = results_df, aes(yintercept = tau_true, col = "Truth"), size = 2)+
  ...
  phi_plot <- ggplot(data = results_df, aes(x = sim_rep, y = phi_est_q50, ymin = phi_est_q025, ymax = phi_est_q975))+
    geom_hline(data = results_df, aes(yintercept = phi_true, col = "Truth"),  size = 2)+
  ...
  Density_plot <- ggplot(data = results_df, aes(x = sim_rep, y = Density_est_q50, ymin = Density_est_q025, ymax = Density_est_q975))+
    geom_hline(data = results_df, aes(yintercept = Density_true, col = "Truth"), size = 2)+
    #geom_errorbar(width = 0, aes(col = "Joint"), alpha = 0.5)+
    geom_point(aes(col = "Joint"))+
...
png(paste0("figures/SimResults_Matrix_tau_",tau_true,"_phi_",phi_true,".png"),width=4,height=6,units="in",res=500)
print(estimate_plot)
dev.off()
